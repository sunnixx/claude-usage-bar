...
enum Win32Icon {
    ...
    static func make(percent: Int?, critical: Bool, stale: Bool) -> HICON? {
        ...
        // Scoped in its own `do` block, deliberately: `colour` MUST be
        // deselected from `memDC` before `CreateIconIndirect` reads it below
        // (a bitmap still selected into a DC is not a valid source for that
        // copy), and it must happen on every exit from this region, not just
        // the one at the bottom. A `defer` registered directly in
        // `make`'s top-level scope would only run once the WHOLE function
        // returns — i.e. after `CreateIconIndirect` — which is the exact bug
        // the previous round of this code had (the `defer` for the
        // `SelectObject` deselect ran too late). Wrapping the
        // select-draw-deselect steps in their own `do` block makes the
        // `defer` inside it run at that block's exit, which happens BEFORE
        // `CreateIconIndirect` unconditionally — including for any early
        // `return` someone might add inside this block later, which is
        // exactly the trap this scoping exists to close off. Do not move
        // the deselect back out to a bare top-level `defer`, and do not add
        // a `return` between the closing `}` of this block and
        // `CreateIconIndirect` without re-selecting `colour` out first.
        do {
        ...
        // `colour` is guaranteed deselected from `memDC` at this point — the
        // `do` block above just ran its `defer`.
...
extension String {
    ...
    /// used from `WindowsLoginItem` in the `Headroom` target.
